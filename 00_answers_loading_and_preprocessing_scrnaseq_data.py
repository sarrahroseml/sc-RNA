# -*- coding: utf-8 -*-
"""00_Answers_Loading_and_preprocessing_scRNAseq_data.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/github/KrishnaswamyLab/SingleCellWorkshop/blob/master/exercises/Preprocessing/notebooks/00_Answers_Loading_and_preprocessing_scRNAseq_data.ipynb
...
data_sqrt = scprep.transform.sqrt(data_norm)After transformation, you should see that the mean expression values are now much
...
Cells with high mitochondrial expression are likely to be dying / burst cells; however, in some cell types we expect healthy cells to have high mitochondrial expression, so this step requires some additional experimental context.

2. Why do we remove genes with non-zero expression in very few cells?

Genes with non-zero expression in very few cells were likely poorly captured by the reverse transcription process. We remove them since we do not have sufficient information to understand their expression across the whole dataset.

3. Why do we normalize expression by library size?

Capture efficiency of the sequencing reagents varies randomly across cells. Although some cells genuinely contain more mRNAs than others, the bulk of the variation in library size is due to the capture efficiency of the sequencing reactions, so we remove this technical difference between cells to better understand their biological similarity.

4. Why do we apply a sublinear transformation (e.g. `sqrt` or `log`) to the normalized expression?

Some genes (like housekeeping genes) naturally exist in a cell in very high numbers of mRNA, while others (like transcription factors) exist in very low numbers. A sublinear transformation decreases the influence on cell similarity of these highly expressed genes.
...
Library size filtering and normalization has the largest effect -- without either of these steps, the output is almost meaningless.

Mitochondrial filtering can also have a very large effect in lower quality datasets, though the cells in this dataset are fairly healthy.
...
They are spread around fairly evenly. This could be different if there was a cell type which had naturally high mitochondrial expression, or if the experiment involved treating cells in such a way that certain cell types were likely to be dying.
